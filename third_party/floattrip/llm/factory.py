...
from third_party.floattrip.core.env import load_local_env
...
def build_chat_llm(*, model: str | None = None, temperature: float = 0) -> Any:
    ...
    if provider == "deepseek":
        from third_party.floattrip.llm.deepseek import build_chat_deepseek
        return build_chat_deepseek(model=model, temperature=temperature)
    elif provider == "doubao":
        from third_party.floattrip.llm.doubao import build_chat_doubao
...
def build_structured_llm(
    ...
    *,
    ...
    provider = resolve_llm_provider()
    if provider == "deepseek":
        from third_party.floattrip.llm.deepseek import build_structured_deepseek
        return build_structured_deepseek(schema, model=model, temperature=temperature)
    elif provider == "doubao":
        from third_party.floattrip.llm.doubao import build_structured_doubao
